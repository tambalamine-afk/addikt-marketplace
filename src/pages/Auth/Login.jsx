  ...
  return (
    <div className="bg-surface-container-lowest text-primary min-h-[calc(100vh-200px)] flex flex-col justify-center items-center p-grid-gutter antialiased w-full">
      <main className="w-full max-w-sm flex flex-col items-center">
        ...
        {/* Title */}
        <h1 className="w-full font-headline-md text-xl md:text-2xl leading-tight text-center uppercase tracking-tight mb-8 whitespace-nowrap" style={{ fontFamily: '"Zalando Sans Expanded", sans-serif', fontWeight: 600 }}>
        ...
        <form className="w-full space-y-6">
          ...
          <div className="flex items-center border-2 border-outline-variant focus-within:border-primary transition-colors bg-surface-container-lowest h-14">
            <span className="px-4 border-r-2 border-outline-variant font-label-caps text-secondary h-full flex items-center bg-surface-container-low font-bold" style={{ fontFamily: '"Zalando Sans Expanded", sans-serif' }}>
              +221
            </span>
            <input className="flex-1 bg-transparent border-none focus:ring-0 px-4 py-2 font-body-sm outline-none placeholder-secondary h-full" placeholder="Numéro de téléphone" type="tel" />
          </div>
          
          {/* Submit Button */}
          <Link to="/verify" className="w-full bg-primary text-on-primary font-button-text py-5 rounded-full uppercase tracking-widest hover:opacity-80 active:scale-95 transition-all duration-200 block text-center font-bold" style={{ fontFamily: '"Zalando Sans Expanded", sans-serif' }}>
            Continuer
          </Link>
          
          {/* Separator */}
          <div className="flex items-center py-4">
            <hr className="flex-1 border-t-[1px] border-outline-variant" />
            <span className="px-4 font-label-caps text-secondary uppercase font-bold" style={{ fontFamily: '"Google Sans", sans-serif' }}>ou</span>
            <hr className="flex-1 border-t-[1px] border-outline-variant" />
          </div>
          
          {/* Google Button */}
          <button className="w-full bg-surface-container-lowest border-2 border-outline-variant text-primary font-button-text py-5 rounded-full flex items-center justify-center space-x-3 hover:bg-surface-container-low active:scale-95 transition-all duration-200 font-semibold whitespace-nowrap" style={{ fontFamily: '"Google Sans", sans-serif' }} type="button">
        ...
        <div className="mt-12 text-center w-full">
          <p className="font-body-sm text-on-surface-variant">
            ...
            <Link to="/register" className="text-accent-orange font-bold hover:underline transition-all">Inscris-toi</Link>
          </p>
        </div>
      </main>
    </div>
