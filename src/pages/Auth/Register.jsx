      ...
      <main className="w-full max-w-md bg-white border-[3px] border-black p-6 md:p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative z-10 my-8">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            ...
            <input className="w-full border-2 border-outline-variant bg-transparent focus:outline-none focus:border-black transition-all font-body-sm p-4 text-on-surface placeholder:text-outline-variant" id="fullName" name="fullName" placeholder="Ton blaze..." required type="text" />
          </div>
          
          {/* Numéro de téléphone */}
          <div className="space-y-2">
            <label className="block font-label-caps text-xs text-on-surface uppercase tracking-widest font-bold" htmlFor="phone" style={{ fontFamily: '"Google Sans", sans-serif' }}>Numéro de téléphone</label>
            <div className="flex relative border-2 border-outline-variant focus-within:border-black transition-all">
              <span className="flex items-center justify-center bg-surface-container-low border-r-2 border-outline-variant px-4 font-button-text text-on-surface font-bold" style={{ fontFamily: '"Zalando Sans Expanded", sans-serif' }}>
                +221
              </span>
              <input className="w-full bg-transparent focus:outline-none font-body-sm p-4 text-on-surface placeholder:text-outline-variant" id="phone" name="phone" pattern="[0-9]{9}" placeholder="77 000 00 00" required type="tel" />
            </div>
          </div>
          ...
          {/* Google Login */}
          <button className="w-full bg-white border-2 border-black text-primary rounded-full py-4 px-6 font-button-text uppercase tracking-widest hover:bg-trust-grey transition-colors flex justify-center items-center gap-3 font-semibold whitespace-nowrap" style={{ fontFamily: '"Google Sans", sans-serif' }} type="button">
        ...
        <div className="mt-8 text-center">
          <Link className="font-button-text text-sm text-on-surface hover:text-accent-rose transition-colors underline decoration-2 decoration-black hover:decoration-accent-rose underline-offset-4 font-bold" style={{ fontFamily: '"Google Sans", sans-serif' }} to="/login">
