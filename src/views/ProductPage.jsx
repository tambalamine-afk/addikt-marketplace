...
export default function ProductPage({ products, toggleLike, addToast }) {
  ...
  const renderProductCard = (p, index, isPopular = false, isNew = false) => (
    <Link key={p.id} href={`/product/${p.id}`} className={`group block ${index === 4 ? 'hidden lg:block' : ''}`}>
    ...
    <>
      <main className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="mb-6 font-label text-sm text-secondary flex items-center gap-2">
          ...
          <Link href={`/category/${product.category?.toLowerCase() || 'femmes'}`} className="hover:text-primary transition-colors text-primary border-b-2 border-primary pb-0.5 capitalize">{product.category || 'Femmes'}</Link>
          <span className="material-symbols-outlined text-[16px]">chevron_right</span>
          ...
          <div className="w-full md:w-[45%] flex flex-col pt-2 lg:pt-8">
            ...
            <div className="mb-8">
              ...
              <div className="flex gap-3">
                <Link href={`/seller/${product.sellerId || 1}`} className="flex-1 bg-white border border-primary text-primary font-bold text-xs uppercase py-3 rounded-full hover:bg-surface-container-high transition-colors text-center block" style={{ fontFamily: '"Zalando Sans Expanded", sans-serif' }}>
                  Voir la boutique
                </Link>
